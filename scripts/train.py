...
from olmo.data import build_train_dataloader, build_dynamic_train_dataloader
...
from olmo.optim import StepCosineHalfWithWarmup
...
def main(cfg: TrainConfig) -> None:
    ...
    # Check for model to load if requeue.
    rq_path = Path(cfg.save_folder) / "latest"  # TODO: is this always the right path to use?
    if rq_path.exists():
        if cfg.requeue:
            cfg.load_path = str(rq_path)
            log.info(f"Requeueing from {cfg.load_path}")
        else:
            log.info("Job already exists but requeue is False.")
            return
    ...
    if cfg.wandb is not None and (get_global_rank() == 0 or not cfg.wandb.rank_zero_only):
        ...
        wandb.init(
            ...
            name=f"{cfg.wandb.name}_sched-{cfg.scheduler.name}_lr-{cfg.optimizer.learning_rate}",
            tags=cfg.wandb.tags,
    ...
    if cfg.scheduler.name == "step_cosine_half_with_warmup":
        decay_steps = StepCosineHalfWithWarmup.get_decay_tokens(cfg.optimizer.learning_rate, total_tokens=int(float(cfg.max_duration[:-1])), alpha_f=cfg.scheduler.alpha_f, lr_decay_factor=cfg.scheduler.lr_decay_factor_for_token_counts, warmup_steps=cfg.scheduler.t_warmup)
        print(f"DECAY STEPS: {decay_steps}")
        # decay_steps = [2, 3]  #TODO: Change this for set_trace
        train_loader = build_dynamic_train_dataloader(cfg, decay_steps=decay_steps)
    else:
...
#
        ...
        if cfg.compile is not None:
            ...
            trainer.train_batch = torch.compile(trainer.train_batch, **cfg.compile.asdict(), dynamic=True)  # type: ignore
...
if __name__ == "__main__":
    ...
    print("testing if log.info is working")
    try:
