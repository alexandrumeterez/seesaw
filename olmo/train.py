...
from olmo.data.iterable_dataset import IterableDataset, FastBSScheduler, batch_by_scheduler
...
from .optim import StepCosineHalfWithWarmup
...
def cross_entropy_loss(
    ...
    # print("BEFORE EXITING LOSS", flush=True)
    # print(f"labels: {labels}", flush=True)
    # print(f"ignore_index: {ignore_index}", flush=True)
    # print(f"(labels != ignore_index): {(labels != ignore_index)}", flush=True)
    # print(f"sum(labels == ignore_index): {(labels == ignore_index).sum()}", flush=True)
    if (labels == ignore_index).sum() > 0:
        print("We have a hit!", flush=True)
...
class Trainer:
    ...
    def get_labels(self, batch: Dict[str, Any]) -> torch.Tensor:
        ...
        # print(f"labels == -100: {(labels==-100).sum()}", flush=True)
        # print(f"keys: {list(batch.keys())}", flush=True)
    ...
    def train_batch(self, batch: Dict[str, Any]) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        ...
        # N = sum([x['input_ids'].size(0) * x['input_ids'].size(1) for x in micro_batches])
        N = sum([x['input_ids'].size(0) for x in micro_batches])
        # sizes = [x['input_ids'].size(0) for x in micro_batches]
        # print(f"BATCHES ARE: {sizes}", flush=True) 
        # In case this helps with memory utilization.
        del batch
        ce_batch_loss = torch.tensor(0.0, device=self.device)
        z_batch_loss = None if not self.cfg.softmax_auxiliary_loss else torch.tensor(0.0, device=self.device)
        print(f"TOTAL MICROBATCHES: {len(micro_batches)}, N = {N}", flush=True)
        # pdb.set_trace()
        index = 0
        for micro_batch in micro_batches:
            with torch.autocast("cuda", enabled=True, dtype=self.cfg.autocast_precision):
                ...
                ce_batch_loss += ce_loss.detach() * batch_size

                # Get loss to optimize for.
                if self.cfg.softmax_auxiliary_loss:
                    assert z_loss is not None
                    assert z_batch_loss is not None
                    loss = (ce_loss + z_loss) * batch_size / N
                    
                    # Update overall Z batch loss.
                    z_batch_loss += z_loss.detach() * batch_size

                else:
                    loss = ce_loss * batch_size / N
        ...
        if z_batch_loss is not None:
            return ce_batch_loss / N, z_batch_loss / N
        return ce_batch_loss / N, None
    ...
    def update_hessian(self, batch: Dict[str, Any]):
        self.optim.zero_grad(set_to_none=True)
        batch = move_to_device(batch, self.device)
        micro_batches = self.split_batch(batch, cut_batch=True)
        for micro_batch in micro_batches:
            with torch.autocast("cuda", enabled=True, dtype=self.cfg.autocast_precision):
                # Run forward pass.
                _, _, logits = self.model_forward(micro_batch, compute_z_loss=self.cfg.softmax_auxiliary_loss)
    ...
    def split_batch(self, batch: Dict[str, Any], cut_batch: bool = False) -> List[Dict[str, Any]]:
        microbatch_size = self.cfg.device_train_microbatch_size
        if cut_batch:
            microbatch_size = microbatch_size // 2
    ...
    def should_eval_this_step(self) -> bool:
        # return False # TODO: remove this set_trace
        assert self.cfg.eval_interval is not None or self.cfg.eval_count_log_scale is not None or self.cfg.eval_token_interval is not None
        if self.cfg.eval_interval is not None:
            ...
            if type(self.cfg.max_duration) == str:
                logspace = np.logspace(-2.1, 0, self.cfg.eval_count_log_scale) * (float(self.cfg.max_duration[:-1]))
            else:
            ...
            # token interval
            return self.global_train_tokens_seen >= self.last_eval_tokens_seen

    def should_save_this_step(self) -> bool:
        return False #TODO
        assert self.cfg.save_interval is not None or self.cfg.save_count_log_scale is not None
        if self.cfg.save_interval is not None:
            return self.global_step % self.cfg.save_interval == 0
        else:
            assert type(self.cfg.max_duration) == int
            logspace = np.logspace(-2.1, 0, self.cfg.save_count_log_scale) * self.cfg.max_duration
            log_steps = [int(n / 100) * 100 for n in logspace]
            return self.global_step in log_steps
    ...
    def should_estimate_hessian_this_step(self) -> bool:
        if self.cfg.optimizer.name == "sophiag" and self.global_step % self.cfg.optimizer.hessian_interval == 0:
            return True
        else:
            return False
    ...
    def eval(self) -> Dict[str, Any]:
        ...
        if self.cfg.optimizer.name == 'adamwschedulefree':
            self.optim.eval()
    ...
    def fit(self):
        ...
        if isinstance(self.scheduler, StepCosineHalfWithWarmup):
            # TODO: Modify this
            self.scheduler.lr_decay_factor = self.cfg.scheduler.lr_decay_factor_for_token_counts
            self.scheduler._build_once(self.cfg.optimizer.learning_rate, self.max_tokens)
            self.scheduler.lr_decay_factor = self.cfg.scheduler.lr_decay_factor
            
            # self.scheduler._decay_tokens = [2, 3] #TODO: Change this for set_trace
            bs_sched = FastBSScheduler(init_bs=self.cfg.global_train_batch_size, milestones=self.scheduler._decay_tokens, gamma=self.cfg.scheduler.batch_size_increase_factor)
            batch_iterator = batch_by_scheduler(iter(self.train_loader), bs_sched, collate_fn=default_collate, drop_last=True, tokens_per_sample=self.cfg.model.max_sequence_length)
        import pdb
            ...
            for epoch in range(self.epoch or 0, self.max_epochs):
                for batch in batch_iterator:
                    ...
                    if self.global_train_tokens_seen >= self.max_tokens:
                        eval_metrics = self.eval()
                        # ADD HERE
                        if self.cfg.eval_token_interval:
                            self.last_eval_tokens_seen += self.cfg.eval_token_interval
                        # Log metrics to W&B.
                        if wandb.run is not None:
                            wandb.log(eval_metrics, step=self.global_step)
                    ...
                    print(f"DEBUGGING BATCH SIZE CONTENT: {batch['index']}", flush=True)
                    ...
                    # TODO: test this, should we not also update on this batch?
                    # TODO: account for this cost in the step counts?
                    if self.should_estimate_hessian_this_step():
                        self.update_hessian(batch)
                    ...
                    if should_log_this_step:
                        ...
                        metrics['total_batches_seen'] = total_batches_seen
                        metrics['learning_rate_test'] = [group["lr"] for group in self.optim.param_groups][0]
                        metrics['last_eval_tokens_seen'] = self.last_eval_tokens_seen
                    ...
                    if save_checkpoints and (
                        cancel_initiated
                        or (self.should_save_this_step() and self.cfg.save_num_checkpoints_to_keep != 0)
                    ):
        ...
        if save_checkpoints:
            if (
                self.cfg.save_num_unsharded_checkpoints_to_keep != 0
