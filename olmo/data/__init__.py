...
from .iterable_dataset import IterableDataset, FastBSScheduler, SimpleStream
...
def build_train_dataloader(train_config: TrainConfig) -> DataLoader:
    ...
    print(f"DATASET SIZE IS: {len(dataset)}", flush=True)
...
def build_dynamic_train_dataloader(train_config: TrainConfig, decay_steps) -> DataLoader:
    assert train_config.device_train_batch_size is not None
    dataset = build_memmap_dataset(train_config, train_config.data, include_instance_metadata=False)
    ...
    barrier()
    seed = train_config.data.seed if train_config.data.seed is not None else train_config.seed
    ds = SimpleStream(dataset, seed=seed, shuffle=True, world_size=get_world_size(), rank=0)
    return DataLoader(
        ds,              # yields dict samples
        batch_size=None,            # single sample per yield
        num_workers=train_config.data.num_workers,
        persistent_workers=None if train_config.data.num_workers == 0 else train_config.data.prefetch_factor,
        prefetch_factor=None if train_config.data.num_workers == 0 else train_config.data.prefetch_factor,
        pin_memory=train_config.data.pin_memory           # True if you want CUDA pinning
    )

    # return DataLoader(
    #     SequenceCountBatcher(
    #         ds,
    #         scheduler=sched,
    #         drop_last=True
    #     ),
