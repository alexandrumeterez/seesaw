...
class OptimizerType(StrEnum):
    ...
    signsgd = "signsgd"
    adamw = "adamw"
    adafactorw = "adafactorw"
    novagradw = "novagradw"
    sgdw = "sgdw"
    sophiag = "sophiag"
    adamwschedulefree = "adamwschedulefree"
    stableshampoo = "stableshampoo"
    stablershampoo = "stablershampoo"
    stablershampoo2 = "stablershampoo2"
    stableradam = "stableradam"
    shampoo_orig = "shampoo_orig"
...
class OptimizerConfig(BaseConfig):
    ...
    # betas: Tuple[float, float] = (0.9, 0.95)  # Second will be ignored by SGD
    beta_0: float = 0.9
    beta_1: float = 0.95
    tie_betas: bool = False
    use_dampening: bool = False  # for SGD
    eps: float = 1.0e-8  # for AdamW
    nesterov: bool = False  # for SGD
    decouple_weight_decay: bool = False  # If true, wd is divided by lr
    hessian_interval: int = 10
    ...
    # Used for adamwschedule free
    use_const_weight: bool = False
    const_weight: float = 0.001
    use_w_avg: bool = False
    w_avg_weight: float = 0.01
    
    # Used for StableShampoo
    shampoo_beta: float = -1
    projection_type: str = "full"
    precondition_frequency: int = 1
    max_precond_dim: int = -1
    
    # Used for OrigShampoo
    
    shampoo_eps: float = 1.0e-15

    shampoo_exp_1: float = 2.5
    shampoo_exp_2: float = 2.5 
...
class SchedulerType(StrEnum):
    ...
    cosine_later_with_warmup = "cosine_later_with_warmup"
    linear_with_warmup = "linear_with_warmup"
    inverse_sqrt_with_warmup = "inverse_sqrt_with_warmup"
    max_scheduler = "max_scheduler"
    constant = "constant"
    step_cosine_half_with_warmup = "step_cosine_half_with_warmup"
...
class SchedulerConfig(BaseConfig):
    ...
    lr_decay_factor: float = 1.0 # lr = lr * lr_decay_factor
    batch_size_increase_factor: float = 1.0 # batch_size = batch_size * batch_size_increase_Factor
    lr_decay_factor_for_token_counts: float = 1.0
...
class TrainConfig(BaseConfig):
    ...
    eval_token_interval: Optional[int] = 30_000_000
    """
    ...
    save_count_log_scale: Optional[int] = None
    """
