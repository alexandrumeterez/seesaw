...
This repository contains the Seesaw learning-rate and batch-size scheduler
research code, based on OLMo, plus a clean-room reproduction of the paper
figures from pinned Weights & Biases runs.

The training tree is an archival snapshot of `opt-olmo` commit
`42b929a41ae0b638766da9e6bb2786b235d33091` from the
`continuous-bs-sched` branch. It is preserved rather than silently repaired so
that the released source remains traceable to the audited implementation. Read
[AUDIT.md](AUDIT.md) before using it for new training: the audit found material
checkpoint/resume, distributed-training, and schedule-boundary issues.

## Reproduce the figures

The primary entry point is
[`reproducibility/reproduce_figures.ipynb`](reproducibility/reproduce_figures.ipynb).
It reproduces Figures 1 and 2 independently of the exploratory notebooks used
to write the paper. The command-line version also renders the supplied
...
Exact W&B run IDs, expected configurations, Git metadata, and the documented
selection policy are in
[`reproducibility/run_manifest.json`](reproducibility/run_manifest.json).
Full history artifacts are read without W&B's sampled `run.history()` API, and
each plotted endpoint is written to a CSV. See
[`reproducibility/README.md`](reproducibility/README.md) for details.
...
The remainder of this README is inherited from the upstream OLMo codebase.
