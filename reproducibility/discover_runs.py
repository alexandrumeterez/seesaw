...
def select_weight_decay(
    ...
    for batch_size in (128, 256, 512):
        common = lambda record, b=batch_size: (
            same_cell(record, model="150M", batch_size=b, learning_rate=3e-3)
            and close(nested_get(record.run.config, "optimizer.weight_decay"), 1e-4)
            and nested_get(record.run.config, "scheduler.adaptive_wd", False) is False
        )
        cosine = latest(
            cosine_records,
            lambda record, predicate=common: predicate(record) and is_cosine(record),
        )
        seesaw = latest(
            seesaw_records,
            lambda record, predicate=common: predicate(record) and is_standard_seesaw(record),
        )
...
def build_manifest(api: wandb.Api) -> dict[str, Any]:
    ...
    return {
        ...
        "selection_policy": {
            ...
            "weight_decay": ("Pin lr=0.003 and configured weight_decay=0.0001, matching the paper."),
        },
        "provenance_warning": (
            "Some runs report a clean Git parent commit in W&B while their output logs "
            "contain code markers absent from that commit. Run IDs and metrics are "
            "pinned, but the exact dirty training diff was not captured."
        ),
        "figures": {
            "figure_1": figure_1,
            "figure_2": figure_2,
