...
def add_tail_inset(
    ...
    inset = inset_axes(parent, width="50%", height="48%", loc=location, borderpad=1)
    plotted = False
    ...
    inset.set_ylabel("")
    inset.grid(True)
    mark_inset(parent, inset, loc1=2, loc2=4, fc="none", ec="0.65", linewidth=0.8)
...
def plot_figure_1(
    ...
    thresholds = {"150M": 2.5e9, "300M": 5.0e9, "600M": 9.0e9}
    inset_locations = {"150M": "upper right", "300M": "upper right", "600M": "upper right"}
    ...
    for column, model in enumerate(MODEL_ORDER):
        ...
        axes[1, column].set_ylim(2.65, 4.0)
        add_tail_inset(
            axes[0, column],
            traces,
            thresholds[model],
...
def plot_weight_decay(
    ...
    role_colors = {"Cosine": "#1f77b4", "Seesaw": "#ff7f0e"}
