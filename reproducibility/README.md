...
This directory independently reproduces the paper figures. It does not import
or execute the exploratory plotting notebooks used during paper development.
...
`appendix_weight_decay`. For the weight-decay plot, the best fixed-weight-decay
cosine pair is selected independently at each batch size and then matched for
Seesaw: $(\eta,\lambda)=(0.003,0.0001)$ for batch sizes 128 and 256, and
$(\eta,\lambda)=(0.01,0.001)$ for batch size 512. Cache files land in
`reproducibility/cache/` and are ignored by Git.
